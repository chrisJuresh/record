 ...
 *
 * The app is files on disk and nothing more: the shell it is opened at, the
...
/**
 * Serves one file of the app, and the shell for the path it is opened at.
 *
...
export async function serveApp(
  ...
  // A server started without the app is the API on its own, which is what a
  // test asking the command what it says over HTTP wants.
  if (directory === undefined) {
    return plainly(response, 404, "this server was not given an app to serve");
  }

  const file = segments.length === 0 ? join(directory, shell) : fileUnder(directory, segments);
